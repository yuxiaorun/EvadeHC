...
class MalwareManipulator(object):
    ...
    def overlay_append(self, seed=None):
        ...
        return (self.bytez + bytes([random.randint(0, upper) for _ in range(L)]), "overlay append, size="+str(L))
    ...
    def imports_append(self, seed=None):
        ...
        return (self.bytez, "imports append. "+libname+" "+funcname)
    ...
    def section_rename(self, seed=None):
        ...
        original_name=targeted_section.name
        ...
        return (self.bytez, "section rename. section:"+original_name+", rename:"+targeted_section.name)
    ...
    def section_add(self, seed=None):
        ...
        self.bytez = self.__binary_to_bytez(binary)
        return (self.bytez, "add random section")
    ...
    def section_append(self, seed=None):
        ...
        if L > available_size:
            L = available_size
        print("available size:",L)
        upper = random.randrange(256)
        targeted_section.content = targeted_section.content + \
            [random.randint(0, upper) for _ in range(L)]

        self.bytez = self.__binary_to_bytez(binary)
        return (self.bytez, "append to section. target section:"+targeted_section.name+" size:"+str(L))
    ...
    def create_new_entry(self, seed=None):
        ...
        self.bytez = self.__binary_to_bytez(binary)
        return (self.bytez, "create new entry. "+entryname)
    ...
    def remove_signature(self, seed=None):
        ...
        return (self.bytez, "remove signature.")
...
def test(bytez):
    ...
    bytez2, disc = manip.overlay_append(bytez)
    print(disc)
    ...
    manip = MalwareManipulator(bytez)
    bytez2, disc = manip.imports_append(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
    ...
    manip = MalwareManipulator(bytez)
    bytez2, disc = manip.section_rename(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
    ...
    print('section_add')
    manip = MalwareManipulator(bytez)
    bytez2, disc = manip.section_add(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
    oldsections = [s.name for s in binary.sections]
    newsections = [s.name for s in binary2.sections]
    ...
    manip = MalwareManipulator(bytez)
    bytez2, disc = manip.section_append(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
    ...
    manip = MalwareManipulator(bytez)
    bytez2, disc = manip.create_new_entry(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
    ...
    manip = MalwareManipulator(bytez)
    bytez2, disc = manip.remove_signature(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
    if binary.has_signature:
    ...
    print('remove_debug')
    manip = MalwareManipulator(bytez)
    bytez2,disc = manip.remove_debug(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
    if binary.has_debug:
    ...
    manip = MalwareManipulator(bytez)
    bytez2, disc = manip.break_optional_header_checksum(bytez)
    print(disc)
    binary2 = lief.PE.parse(bytez2)
...
def modify_without_breaking(bytez, actions=[], seed=None):
    ...
    for action in actions:
        ...
        def helper(_action,shared_list, paths):
            ...
            try:
                shared_list[:], disc = _action(seed)
                shared_list.append(disc)
        ...
        # define process
        p = multiprocessing.Process( target=helper, args=(_action,shared_list, paths) )
        ...
        try:
            ...
            print('==== timeouterror ')
            p.terminate()
        paths.append(shared_list[-1])
        bytezs.append(array.array('B', shared_list[:-1]).tobytes())
        bytez = array.array('B', shared_list[:-1]).tobytes() # copy result from child process
...
def generate_random_actions(length):
    actions=[]
    for i in range(length):
        actions.append(np.random.choice(list(ACTION_TABLE.keys())))
    return actions
